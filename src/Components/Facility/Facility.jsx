...
export default function facility() {
  ...
  <span className="flex items-center">
    ...
    <span className="text-sm sm:text-lg">◇</span>
        ...
        <h2 className="text-6xl font-serif text-gray-900">Hotel Facilities</h2>
      </div>

      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-9 ">
  {facilities.map((facility, index) => (
    ...
    >
      {facility.icon}
      <h3 className="text-xl mt-5  font-serif font-medium text-gray-900 mb-2">
        {facility.title}
      </h3>
      <p className="text-gray-600  font-serif  text-sm mt-5">{facility.description}</p>
    </div>
  ))}
</div>
    </div>
